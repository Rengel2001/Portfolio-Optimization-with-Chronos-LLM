...
class Agent(torch.nn.Module):
    ...
    def forward(self, inputs_embeds):
        ...
        portfolio_weights = torch.nn.functional.softmax(x, dim=-1).unsqueeze(0)  # Shape: (1, num_assets)
...
class Portfolio_Optimization_Strategy():
    ...
    def backtest(self, dataframe):
        ...
        for y in years[:-1]:
            ...
            self.optimizer = torch.optim.AdamW(self.agent.parameters(), lr=1e-2)

            # Train up to year t (with either expanding window if <3 years, else rolling 3-year window) 
            self.train(dataframe, epochs=5, current_year=y, start_year=years[0], window_length=3)
    ...
    def train(self, df, epochs, current_year, start_year, window_length):
        ...
        if (current_year - start_year + 1) < window_length:
            # Use expanding window from start_year to t
            ...
            if day in BOM_dates:
                ...
                if portfolio_ids is not None and weights is not None:
                    ...
                    if weights is not None and returns is not None:
                        portfolio_return = torch.dot(weights, returns).item()
    ...
    def loss_function(self, weights, returns):
        ...
        entropy = -torch.dot(weights, log_weights)
        eta = 1e-2
        loss = (-expected_return + eta * entropy).to(weights.device)
        return loss
...
if __name__ == '__main__':
    ...
    qs.reports.html(returns_series, benchmark=spy_returns, output='../results/long_only_backtest.html')
    ...
    # Save to CSV
    output_csv_path = '../results/long_only_backtest.csv'
